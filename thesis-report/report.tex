...
\end{align}
and constructing $\mathbf{K}$ involves choosing a kernel function mapping $k: \mathcal{X} \times \mathcal{X} \rightarrow \mathbb{R}$ such that each element of the matrix is the evaluation
\begin{align}
...
Appendix (\ref{appendix:positive-definite-kernel}) shows that choosing kernel functions that are defined as inner products of a feature space mapping will ensure that $\mathbf{K}$ is positive semi-definite for a valid covariance matrix.
...
In GP regression, the Bayesian posterior for a test point $x \in \mathcal{X}$ when conditioned on training data acts as a `prediction' for the epistemic uncertainty of the test data responses. With all terms being Gaussian, this posterior has a closed-form conditional Gaussian expression.
...
\end{align}
with $P \coloneqq \mathbb{P}(Z)$ the prior, $p(y) = \int p(y\vert z)p(z) dz$ the marginal log-likelihood, $\mathbb{E}_{Q}\left[\wc \right]$ denoting the expectation with respect to $Q$, and $\KLD\left[\wc, \wc\right]$ denoting the KL divergence.
It follows that
\begin{align}
...
\end{align}
with $\left[\alpha_1, \dots \alpha_N\right]^T = \mathbf{K}_{\mathbf{X}, \mathbf{X}}^{-1}\mathbf{Y}$.
...
\end{align}
with $\left[\beta_1, \dots \beta_M\right]^T = \mathbf{K}_{\mathbf{Z}, \mathbf{Z}}^{-1}\boldsymbol{\mu}$.
...
The likelihood evaluation of BNNs are also most definitely mis-specified, as it's unlikely that evaluating such over-parameterised models would provide any meaningful insights into the data likelihood. 
...
This section will review \cite{wild2022generalized}, which extends GVI to infinite dimensional settings for quantifying uncertainties on function spaces, providing a framework for learning variational GPs.
