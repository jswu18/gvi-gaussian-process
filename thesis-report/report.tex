...
For example, a larger approximation space $\boldsymbol{Q}$ can cause $L(Q)$ to be intractable or cause a more difficult optimisation space.
...
We will review \cite{burt2020convergence}, which proposes an iterative selection procedure that greedily chooses the next inducing point based on the highest marginal variance in the prior when conditioned on the currently selected set of inducing points
\begin{align}
...
This will be followed by a review of \cite{wild2022generalized} showing that extending GVI to an infinite dimensional setting for GPs constructs a learning framework that can accommodate richer GP approximation spaces by avoiding the KL divergence from standard VI.
...
\end{enumerate}
which are often violated in settings of larger-scaled modelling like Bayesian Neural Networks (BNNs). For example in BNNs, Gaussian priors are generally chosen for computational convenience and are most likely mis-specified in the Bayesian context. Over-parameterised BNNs are also most definitely mis-specified. It is unreasonable to interpret their evaluation as any meaningful insight into the data likelihood. Finally, the posterior of BNNs are often approximated either through samplers or variational approximations. To achieve sampling convergence, larger-scaled models can often require much more computational resources and time than is practically available. On the other hand, achieving a reasonable variational approximation is not always guaranteed, as discussed in Section \ref{section:vi-in-finite-dimensions}.
...
\cite{knoblauch2022optimization} also shows that by generalising the Bayesian posterior within the learning framework of (\ref{gvi-posterior}), any choice of prior $P$ for $Z$, valid divergence $\mathbb{D}$, loss $\ell$, and approximation set $\boldsymbol{Q}$ will result in a generalised posterior solution that maintains interpretations as a belief update, within this generalised Bayesian framework. 
This flexible inference approach will motivate the replacement of the KL divergence that was problematic in standard VI as reviewed in Section \ref{section:vi-gp}.
...
\cite{wild2022generalized} explains that the Kolomogorov Extension Theorem ensures for a given GP, there exists a corresponding Gaussian measure (GM) on the trivial function space $\mathcal{F} = \left\{f: \mathcal{X} \rightarrow \mathbb{R}\right\}$. 
However, this space is highly prone to support mis-match for the KL-divergence and ensuring a tractable KL on $\mathcal{F}$ requires heavy restrictions on the variational family $\boldsymbol{Q}$, as discussed in section \ref{section:svgp}. 
Instead, \cite{wild2022generalized} identifies the existence of GPs with corresponding GMs on the Hilbert space of square integrable functions $\mathcal{L}^2\left(\mathcal{X}, \rho, \mathbb{R}\right)$ when the mean function satisfies 
\begin{align}
...
\begin{align}
    ...
    \label{l2-kernel}
\end{align}
also known as a trace-class kernel.
These conditions guarantee sample functions from the GP to have square-integrable paths, such that there exists $\mathbb{P} \coloneqq \mathcal{N}(m, C)$, a GM on $\mathcal{L}^2\left(\mathcal{X}, \rho, \mathbb{R}\right)$ with the same mean $m$ and a covariance operator
\begin{align}
    C(f) \coloneqq \int_{\mathcal{X}} k(\cdot, x) f(x) d\rho(x)
\end{align}
for any function $f \in \mathcal{L}^2\left(\mathcal{X}, \rho, \mathbb{R}\right)$.
...
\begin{align}
    \mathbb{W}\left[Q, P\right]^2 = \left\|m_P - m_Q\right\|_2^2 + \tr\left(C_P\right) + \tr\left(C_P\right) - 2 \tr \left[\left(\left(C_P\right)^{1/2} C_Q \left(C_P\right)^{1/2} \right)^{1/2}\right]
\end{align}
which is always well-defined and does not have the support mis-match problem of the KL divergence. 
The subscripts denotes the respective GM of the object, $\tr$ is the trace of an operator, and $(C)^{1/2}$ denotes the square root of the positive self-adjoint operator $C$.
Most mean functions satisfy (\ref{l2-mean}) and (\ref{l2-kernel}), ensuring the existence of P and Q on $\mathcal{L}^2\left(\mathcal{X}, \rho, \mathbb{R}\right)$.
...
With GWI, \cite{wild2022generalized} trains GWI-net, construcing the variational set by parameterising the mean with a neural network, replacing the svGP mean in (\ref{svgp-mean}), showing the potential of this approach.
