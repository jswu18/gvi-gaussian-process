...
\caption{Inducing Point Selection of MNIST with Different Kernels}\label{mnist-inducing-points}
\end{figure}
...
We can see that with the NNGP kernel, Algorithm \ref{alg:inducing-point-selection} is significantly better at selecting different instances of the same digit, providing better coverage of data variation.
On the other hand, images selected with the ARD kernel is comprable to random selection. 
...
\begin{class}[text width=5cm]{ApproxGPClassification}{-3, -3}
\inherit{ApproxGP}
...
\begin{class}[text width=5cm]{ApproxGPRegression}{9, -3}
\inherit{ApproxGP}
\inherit{GPRegression}
\end{class}
