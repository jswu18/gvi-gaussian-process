...
\end{itemize}
to construct the objective in (\ref{gvi-posterior-in-fs}). \cite{wild2022generalized} constructed GWI-net with $m_P$ as the zero mean function, $k$ as the ARD kernel, $m_Q$ as a neural network, and $r$ as the svGP kernel
\begin{align}
...
The GWI-net construction from \cite{wild2022generalized} uses a ARD kernel for the prior
\begin{align}
...
\end{minipage}%
\caption{Inducing Point Selection with Different Prior Kernels}\label{mnist-inducing-points}
\end{figure}

We show some preliminary results in Figure \ref{mnist-inducing-points}, where we compare inducing point selection with different kernels on the MNIST data. 
...
We can see that with the NNGP kernels, Algorithm \ref{alg:inducing-point-selection} is significantly better at selecting different instances of the same digit, providing better coverage of data variation.
...
In our experiments, we choose non-stationary kernels for $k_0$ to ensure non-zero gradients for the gram matrix diagonals $k(h(x), h(x))$. 
...
Figure \ref{inducing-points-and-kernel} visualises the inducing points selected for the first four iterations of Algorithm \ref{alg:inducing-points-prior-gp} overlayed with the Bayesian posterior of a zero mean GP parameterised with the learned kernel, conditioned on those inducing points. 
...
We loosen this interpretation with the optimisation-centric nature of GVI, viewing this regulariser as potentially better informed than the prior GP.
...
\end{align}
Tempering is also applied to classification by constructing $\boldsymbol{\alpha} \coloneqq \left[\alpha_T^1 \dots \alpha_T^J\right]^T \in [0, \infty)^J$, a seperate tempering factor for each GP, and optimising the classification log-likelihood in (\ref{robust-max-function-expected-log-likelihood}).
...
We will present projected regularisations, followed by different formulations for the projected GVI objective.
...
These provide different formulations for the pGVI objective.
...
Because there is currently no existing stable, well-maintained, and well-documented implementation of GPs with JAX, all implementations were essentially developed from scratch for the purposes of this project.
...
\subsection{Experiments in Different Data Domains}
...
\newpage
\subsection{Example Regulariser Gaussian Process Training Configuration}\label{appendix:regulariser-yaml}
...
\newpage
\subsection{Example Approximate Gaussian Process Training Configuration}\label{appendix:approximate-yaml}
