...
Building on \cite{wild2022generalized}, we propose the general GVI-GP objective:
\begin{align}
...
\begin{align}
    ...
    \right\}
\end{align}
containing all valid mean and kernel functions that ensure the existence of a GM on the $\mathcal{L}(\mathcal{X}, \rho, \mathbb{R})$ Hilbert space. In practice, we choose a subset $\boldsymbol{Q}_{\boldsymbol{\Theta}} \subset \boldsymbol{Q}_{\mathcal{L}^2}$:
\begin{align}
...
\end{align}
where $\boldsymbol{\Theta}^m$ is the mean function parameter space, $\boldsymbol{\Theta}^k$ is the kernel function parameter space. $\boldsymbol{Q}_{\boldsymbol{\Theta}}$ remains a very general approximation space. For example if we choose the mean parameter space $\mathbb{R}^M$ and the kernel parameter space $\mathbb{R}^{M\times M}_{\succ 0}$ and inducing points $\left\{x_m, y_m\right\}_{m=1}^{M} \subset \{x_n, y_n\}_{n=1}^{N}$ we can recover the svGP parameter space $\boldsymbol{\Gamma}$, assuming the svGP has a corresponding GM on $\mathcal{L}(\mathcal{X}, \rho, \mathbb{R})$. 

Defining an approximation space $\boldsymbol{Q}_{\mathcal{L}^2}$ ensures that GVI-GPs are extremely flexible, with the potential to learn in complex problem domains traditionally dominated by neural network architectures.
By choosing variational kernels of $\mathcal{O}(M^3)$ computational complexity, we ensure the scaleability of GVI-GPs. 
In Section \ref{section:gvi-gp-construction}, we present different choices of mean and kernel functions with corresponding parameter spaces $\boldsymbol{\Theta}^m$ and $\boldsymbol{\Theta}^k$ respectively. 
We will also propose different choices of discrepancies and priors for the regulariser $\D\left[\cdot, \mathbb{P}_{\mathcal{L}^2}\right]$ and loss $\ell(y_n, x_n)$ terms in (\ref{gvi-gp-objective}) to demonstrate the flexibility of learning GVI-GPs within this framework.
...
\begin{align}
            r(x,x') := r_0(x,x') - r_0(x,Z) \big( r_0(Z,Z) \big)^{-1} r_0(Z,x)
\end{align}
where we can parameterise the base kernel $r_0$ however we want, i.e. with a deep NN kernel or a NNGP kernel.
