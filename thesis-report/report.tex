...
{\bf Disclaimer:} 
This report is submitted as part requirement for the Master's in Computational Statistics \& Machine Learning Degree at University College London (UCL). It is
...
\\\jw{Talk about the difficulties with choosing the correct kernel and discuss that more expressive kernels like the NNGP and deep kernels will be introduced in later sections}
...
\begin{align}
    ...
    % \\ &= \log \int_{\mathbb{R}^{N^*}} P_{\GP} \left(\mathbf{Y}_N \vert Y_{N*}, \mathbf{X}_{N}, \mathbf{X}_{N*}, \sigma^2\right) P_{\GP}\left(Y_{N*}\vert \mathbf{X}_{N*}, \sigma^2 \right) d Y_{N*}
    \label{log-like}
\end{align}
...
This section reviews Generalised Variational Inference (GVI) as introduced by \cite{knoblauch2022optimization} in the context of GPs, showing that a variational approximation learned from (\ref{elbo-kld}) involving the KL divergence and Bayesian posterior can be interpreted as a special case within a more general learning framework. This is followed by a review of \cite{wild2022generalized} showing the feasibility of GVI as a GP learning framework.

\subsection{Generalising Bayesian Variational Inference}
Statistical modelling is traditionally focused on characterising an underlying data generation process. In a Bayesian context, we use the prior belief regression model $P_{\GP}\left(Y_{N*}\vert \mathbf{X}_{N*}, \sigma^2\right)$ and the likelihood of our observations $P_{\GP}\left(\mathbf{Y}_N|\mathbf{X}_N, \sigma^2\right)$ derived from the likelihood model $y_n \sim F(x_n) + E$ introduced in (\ref{regression-data-uncertainties}) to update our model beliefs: 
\begin{align}
...
\end{align}
where $P_{\GP}^B$ is the Bayesian posterior, an update on the prior belief of the data generation process $P_{\GP}$. The validity of the Bayesian posterior relies on three assumptions concerning the prior, the likelihood, and the normaliser. However, in practical use cases, GPs are constructed for predictive performance rather than model specification. As such, GP learning approaches typically violate the assumptions meaning that it is no longer reasonable to view (\ref{bayesian-posterior}) as a valid Bayesian belief update.
