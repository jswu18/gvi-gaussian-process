...
We will also define new notation such as $P \coloneqq \mathbb{P}(F)$ and $Q \coloneqq \mathbb{Q}(F)$, whenever convenient. We will use 
\begin{align}
    \mathbb{D}[Q, P]
\end{align}
to denote a divergence between the measures $Q$ and $P$, where a divergence satisfies
\begin{align}
    \mathbb{D}[Q, P] \geq 0
\end{align}
and 
\begin{align}
    \mathbb{D}[Q, P] = 0 \Leftrightarrow Q = P.
\end{align}
Other divergences will follow similar notation and will be defined whenever they are used.

Other conventions that we follow include:
\begin{itemize}
    \setlength\itemsep{-0.25em}
    \item $\mathbb{E}_{P}[\wc]$ to denote expectation with respect to $P$, and
    \item $\left[\wc\right]^T$ to denote the matrix transpose. 
\end{itemize}
 

\newpage
\section{Introduction}
Uncertainty quantification (UQ) is an important research direction within the machine learning community.
As an approach, UQ can demonstrate desirable prediction behaviours such as out-of-distribution (OOD) detection. 
...
This computes divergences between scalar projections of GPs, providing significantly cheaper linear time computational costs compared to the cubic costs of existing approaches.
...
Gaussian processes (GPs) are powerful function approximators that can be used for both regression and classification tasks. The following sections will review GPs following \cite{rasmussen2003gaussian}, \cite{matthews2017scalable}, and \cite{wild2022generalized}.
...
\begin{align}
    y_n \sim F(x_n) + \epsilon_n,
    \label{regression-data-uncertainties}
\end{align}
where the GP random function mapping $F$ accounts for the epistemic (model) uncertainty and the random scalar $\epsilon_n$ accounts for the aleatoric (measurement) uncertainty. In this formulation, we assume that the aleatoric uncertainty is homoscedastic of the form
\begin{align}
    \epsilon_n \sim \mathcal{N} \left(0, \sigma^2\right).
...
\end{align}
where $\mathbf{m}_{\mathbf{X}}$ is more verbose notation for (\ref{gp-mean-vector}) and $\mathbf{K}_{\mathbf{X}, \mathbf{X}}$ is verbose for (\ref{gp-kernel-matrix}). $\mathbf{K}_{x, \mathbf{X}} \in \mathbb{R}^{1 \times N}$ and $\mathbf{K}_{\mathbf{X}, x} \in \mathbb{R}^{N \times 1}$ are row and column matrices respectively, constructed with $\mathbf{X}$ and $x$ following (\ref{gp-kernel-matrix}). $\mathbf{I} \in \mathbb{R}^{N \times N}$ is the identity matrix.
...
\begin{algorithmic}
\State $m \leftarrow 1$
  \State $z_1 \leftarrow \argmax_{x \in \mathbf{X}} k(x, x)$
 \State $\mathbf{z} \leftarrow \{z_1\}$ \Comment{initialise selection set}
\While{$m < M$}
\State$z_{m+1} \leftarrow \argmax_{x \in \mathbf{X}} \left(\diag \left(\mathbf{K}_{\mathbf{X}, \mathbf{X}} - \mathbf{K}_{\mathbf{X}, z_{m}} k\left(z_{m}, z_{m}\right)^{-1}\mathbf{K}_{z_{m}, \mathbf{X}}\right)\right)$
 \State  $\mathbf{z} \leftarrow \mathbf{z} \cup \{z_{m+1}\}$ \Comment{add to the current selection set}
...
\Require $\theta_0$ \Comment{initial hyper-parameters of $k$}
 \State $i \leftarrow 0$
  ...
  \State $\mathbf{U}_0 \leftarrow \mathbf{Y}\left[\mathbf{X}.\text{index}\left(\mathbf{Z}_0\right)\right]$ \Comment{corresponding inducing point responses}
\While{$\mathbf{Z}_i \neq \mathbf{Z}_{i-1}$}
...
In the loop of Algorithm \ref{alg:inducing-point-selection}, each element of the diagonal evaluation has time complexity  $\mathcal{O}(M)$ from matrix multiplication, so computing for all $N$ candidate points along the diagonal is $\mathcal{O}(NM)$.
...
Selecting $M$ inducing points, Algorithm \ref{alg:inducing-point-selection} has $\mathcal{O}(NM^2)$ time complexity .
Choosing $M$ as $\mathcal{O}(N^{1/2})$, we have $\mathcal{O}(N^{2})$ time complexity  and $\mathcal{O}(N)$ space complexity. 
Assuming Algorithm \ref{alg:inducing-points-prior-gp} loops for $k$ iterations, we have $\mathcal{O}(k N^{2})$ time complexity and the same space complexity as Algorithm \ref{alg:inducing-point-selection}.
...
Instead, \cite{wild2022generalized} identifies the existence of GPs with corresponding GMs on the Hilbert space of square integrable functions $\mathcal{L}^2\left(\mathcal{X}, \rho, \mathbb{R}\right) = \left\{f \in \boldsymbol{\mathcal{F}} : \left[\int_{\mathcal{X}} f(x)^2 d\rho(x)\right]^{1/2} < \infty\right\}$ when the mean satisfies
\begin{align}
...
We will denote $\mathcal{L}^2\left(\mathcal{X}, \rho, \mathbb{R}\right)$ as $\mathcal{L}^2$ for convenience. 
...
We call this the \textit{Cholesky parameterisation}. 
...
We introduce a new parameterisation of $\mathbf{\Sigma}$ called the \textit{diagonal parameterisation}. The diagonal parameterisation learns
\begin{align}
...
This suggests that a Cholesky decomposition may be more expressive than needed for our purposes.

We also propose a \textit{base kernel parameterisation} replacing $\mathbf{K}_{x, \mathbf{Z}} \mathbf{\Sigma} \mathbf{K}_{\mathbf{Z}, x}$, the last term of (\ref{gwi-svgp-kernel}) such that
\begin{align}
    r(x, x') = k(x, x') - \mathbf{K}_{x, \mathbf{Z}} \left( \mathbf{K}_{\mathbf{Z}, \mathbf{Z}}\right)^{-1} \mathbf{K}_{\mathbf{Z}, x'} + r_0(x, x'),
\end{align}
where $r_0$ is the base kernel, which can be any kernel function. We learn the hyper-parameters of $r_0$ with GWI through gradient-based optimisation.

\paragraph{Sparse Posterior Kernels}
Inspired by the GP posterior covariance in (\ref{gp-posterior-covariance}) and the svGP kernel from \cite{titsias2009variational}, we present a new form of variational kernel we call \textit{sparse posterior kernels} with the formulation
\begin{align}
...
During GP prediction with sparse posterior kernels, the inversion $r_0\left(\mathbf{Z}, \mathbf{Z}\right)^{-1}$ is a constant. 
However during inference, gradients must pass through it to learn the parameters of the base kernel. 
Thus, we also present a \textit{fixed} version of the sparse posterior kernel
\begin{align}
...
\end{algorithmic}
\end{algorithm}
\paragraph{The Regulariser} The prior is an essential component of a Bayesian posterior.
...
\end{align}
where $N_B < N$ and $N_S < N$ for two independent batches. $\lambda_{n}: \mathbb{R}^{N_S \times N_S} \rightarrow \mathbb{R}$ evaluates the $n^{th}$ eigenvalue. We introduce further numerical approximations of (\ref{wasserstein-distance-approximation}) to improve computation speed.
...
This suggests that for our use cases, the correlation constraints imposed by the eigendecomposition is a less essential contributor to achieving good prediction performance.
...
In most VI use cases where $Q$ is an approximation, we would rarely expect to actually be in a setting where $Q$ matches our target posterior. 
% \vw{I think you dont need the previous sentence. Your point still holds without it and it is probably a bit controversial to say this.}
...
\caption{Mean Function Implementation Architecture}
...
\caption{Kernel Function Implementation Architecture}
