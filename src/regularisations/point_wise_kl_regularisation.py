...
class PointWiseKLRegularisation(RegularisationBase):
    def __init__(
        self,
        gp: GPBase,
        regulariser: GPBase,
    ...
    def calculate_pointwise_kl_divergence(
        m_p: jnp.ndarray,
        c_p: jnp.ndarray,
        m_q: jnp.ndarray,
        c_q: jnp.ndarray,
            ...
            jnp.mean(
                jax.vmap(
                    lambda m_p_, c_p_, m_q_, c_q_: (
                        jnp.log(jnp.sqrt(c_q_) / jnp.sqrt(c_p_))
                        + (c_p_ + jnp.square(m_p_ - m_q_)) / (2 * c_q_)
                        - 0.5
                    )
                )(m_p, c_p, m_q, c_q)
            )
    ...
    def _calculate_regularisation(
        ...
        return jnp.mean(
            jax.vmap(
                lambda m_p, c_p, m_q, c_q: PointWiseKLRegularisation.calculate_pointwise_kl_divergence(
                    m_p=m_p,
                    c_p=c_p,
                    m_q=m_q,
                    c_q=c_q,
