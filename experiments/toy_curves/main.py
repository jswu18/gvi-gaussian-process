...
def train_regulariser(config: Dict, output_path: str, experiment_name: str) -> None:
    ...
    for curve_function in CURVE_FUNCTIONS:
        ...
        plot_prediction(
            ...
            gp=regulariser_gp,
            gp_parameters=regulariser_gp_parameters,
            title=f"Regulariser GP: {curve_function.__name__}",
            save_path=os.path.join(
                save_path,
                experiment_data.name,
                "prediction.png",
        ...
        plot_losses(
            ...
            labels=[
                f"iteration-{i}" for i in range(len(regulariser_post_epoch_histories))
            ],
            loss_name=config["empirical_risk_schema"],
            title=f"Regulariser GP Empirical Risk: {curve_function.__name__}",
            save_path=os.path.join(
                save_path,
                experiment_data.name,
                "losses.png",
...
def train_approximate(config: Dict, output_path: str, experiment_name: str) -> None:
    ...
    regulariser_config_path = os.path.join(
        config_directory_parent_path,
        "configs",
        ActionSchema.train_regulariser,
        f"{config['regulariser_name']}.yaml",
    )
    with open(regulariser_config_path, "r") as regulariser_config_file:
        regulariser_config = yaml.safe_load(regulariser_config_file)
    data_path = construct_path(
        output_path=output_path,
...
def temper_approximate(config: Dict, output_path: str, experiment_name: str) -> None:
    ...
    regulariser_config_path = os.path.join(
        ...
        "configs",
        ActionSchema.train_regulariser,
        f"{approximate_config['regulariser_name']}.yaml",
    )
    with open(regulariser_config_path, "r") as regulariser_config_file:
        regulariser_config = yaml.safe_load(regulariser_config_file)
    data_path = construct_path(
    ...
    for curve_function in CURVE_FUNCTIONS:
        ...
        plot_prediction(
            ...
            save_path=os.path.join(save_path, experiment_data.name, "prediction.png"),
        )
        plot_losses(
