...
\section{Loss Minimisation $\rightarrow$ GVI \cite{wild2023rigorous}}
...
\end{align}
where minimisers of (\ref{risk-minimisation}) $\hat{q}$, can correspond to minimisers of (\ref{loss-minimisation}) $\hat{\theta}$, by defining the Dirac measure $\hat{q}(\theta) = \delta_{\hat{\theta}} (\theta)$. This first reformulation changes a non-convex problem with respect to $\theta$ to a linear problem with respect to $q$. To show this, consider two minimisers $\theta_A$ and $\theta_B$ such that:
\begin{align}
...
\begin{align}
    ...
    &= \min_{Q \in \mathcal{P}(\mathbb{R}^J)} \int \left( \sum_{n=1}^N\ell_n(x_n, \theta)\right) dq(\theta)
\end{align}
where ($\ref{show-linear-linear-operator}$) follows by linearity and ($\ref{show-linear-minimisers}$) follows from ($\ref{ex-risk-minimisers}$), showing that ($\ref{risk-minimisation}$) is a linear problem in $Q$. 
...
\end{align}
where $\lambda > 0$. The solution of ($\ref{regularised-risk-minimisation}$) is no longer necessarily a minimiser of ($\ref{risk-minimisation}$). Rather, $\lambda$ balances the tradeoff between the empirical risk minimisation of ($\ref{risk-minimisation}$) and deviance from a prior measure $\pi$, which in this context we can view as a reference measure.\\
\newline 
Choosing $\Pi =\mathcal{P}(\mathbb{R}^J)$, $\ell(\theta) = \sum_{n=1}^N\ell_n(x_n, \theta)$, and $D(q\| \pi) = \lambda D_r(q\| \pi)$, we see how ($\ref{regularised-risk-minimisation}$) fits into the general form of ($\ref{general-posterior}$). 


\jw{I think I'm missing something fundamental in my understanding here. What are the implications of showing that that we can also arrive at the general posterior through this convexification process? That both this and the Bayesian posterior are forms of a more general problem?}

% One practical issue with this problem formulation is over-fitting to the empirical data. With a heavily over-parameterised model, there may exist $\hat{\theta} \in \Theta$ such that:
% \[f_{\hat{\theta}}(x) = \begin{cases}
% y_n ,  & \text{if } x=x_n, n=1,\cdots,N\\
% 0 , & \text{otherwise}\\
% \end{cases}\]
% Clearly $f_{\hat{\theta}}$ is unable to generalise beyond the training data and is severly overfit. \\
% \newline
% Another issue is the lack of uniqueness guarantees for $\theta^*$, which are necessary to prove theoretical results about the model parameter space $\Theta$ in learning theory. $f_{\theta}$ is typically an over-parameterised neural network causing $\ell(\theta)$ to be a highly non-convex surface. 
% In practise, the guaranteed existence of a unique minimiser $\theta^*$ isn't necessary, as good local minimisers often achieve reasonable predictive performance. 
% Although this formulation is widely used in practise, it does not guarantee uniqueness

% We can seperately arrive at (\ref{general-posterior}) through the probabilistic lifting and convexification of a general loss minimisation problem:
